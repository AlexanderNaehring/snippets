...
if (isset($matrnr) && isset($passwd)) {
	...
	/* request calendar */
	$calParams = array(
		'startdt'	=> strftime('%d.%m.%Y', time() - 6*31*24*60*60),  /* halbes Jahr Vergangenheit */
		'enddt'		=> strftime('%d.%m.%Y', time() + 6*31*24*60*60)   /* halbes Jahr Zukunft */
	);
	...
	foreach ($lines as $line) {
		if ($line) {
			...
			switch ($key) {
				...
				case 'LOCATION':
					$matches = array();
					if (preg_match('/^([0-9]+\|[0-9A-Za-z]+)/', $value, $matches)) {
						$room = $matches[1];
